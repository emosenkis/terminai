...
# Historical Homebrew Versions

The Homebrew tap MUST support installing historical Terminai releases, not only
the latest release. For every release, update the latest `Formula/terminai.rb`
and also add or update a versioned formula named
`Formula/terminai@<version>.rb` in the tap, where `<version>` is the semantic
version without the leading `v`, for example `Formula/terminai@0.1.2.rb`.
The versioned formula must point at that exact GitHub release asset and use the
matching SHA-256 hashes from the built release artifacts. Do not delete older
versioned formulae.
