...
fn main() {
    ...
    if !node_modules.exists() {
        ...
        match status {
            Ok(s) if s.success() => {
                println!("cargo:warning=TypeScript dependencies installed successfully");
            }
            Ok(s) => {
                // Try npm as fallback
                println!(
                ...
                match npm_status {
                    Ok(s) if s.success() => {
                        println!("cargo:warning=TypeScript dependencies installed with npm");
                    }
                    _ => {
    ...
    if build_succeeded && dist_file.exists() {
        println!("cargo:rustc-env=TERMINAI_AGENT_JS_PATH={}", dist_file.display());
        println!("cargo:rustc-cfg=has_bundled_agent");
    } else {
        println!("cargo:warning=Using fallback embedded agent (TypeScript build not available)");
    }
}
